...
export function PricingCard({
  ...
  return (
    <motion.article whileHover={{ y: -10 }} className="relative group h-full">
      {featured ? (
        <div className="absolute -inset-1 rounded-[2.5rem] bg-linear-to-r from-silver-400 via-mystic-500 to-silver-300 blur opacity-25 transition-opacity group-hover:opacity-45" />
      ) : null}

      <div
        className={cn(
          'relative flex h-full flex-col items-center rounded-[2.5rem] border p-10 text-center transition-all',
          featured
